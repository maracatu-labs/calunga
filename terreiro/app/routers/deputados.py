...
router = APIRouter(prefix="/v1/deputados", tags=["deputados"])
...
async def listar_deputados(
    ...
    partido: str | None = Query(None, description="Sigla do partido (ex: PT, PL)"),
    ...
    rows = await parlamentares_q.listar_parlamentares(
        pool, tipo="deputado", uf=uf, partido=partido, nome=nome, limit=limit, offset=offset
    )
    data = [ParlamentarResponse(**dict(r)) for r in rows]
    return ParlamentarList(data=data, total=len(data))

@router.get("/{deputado_id}", response_model=ParlamentarResponse)
async def buscar_deputado(deputado_id: uuid.UUID):
    pool = get_pool()
    row = await parlamentares_q.buscar_parlamentar(pool, deputado_id)
    if not row:
        raise HTTPException(status_code=404, detail="Deputado não encontrado")
    return ParlamentarResponse(**dict(row))

@router.get("/{deputado_id}/despesas", response_model=DespesaList)
async def listar_despesas_deputado(
    deputado_id: uuid.UUID,
    ...
    parlamentar = await parlamentares_q.buscar_parlamentar(pool, deputado_id)
    if not parlamentar:
        raise HTTPException(status_code=404, detail="Deputado não encontrado")

    rows = await despesas_q.listar_despesas(
        pool, parlamentar_id=deputado_id, ano=ano, mes=mes, categoria=categoria, limit=limit, offset=offset
    )
